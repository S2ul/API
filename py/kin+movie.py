...
ans=input("지식인과 영화 중 택 1: ")
naver_client_id = "roUBcoDvfgW4Tu2flyLu"
naver_client_secret = "jGww9cGsvh"
if ans == '영화' :
    def get_movie_count(query,display):
        encode_query=urllib.parse.quote(query)

        search_url ="https://openapi.naver.com/v1/search/movie?query=" + encode_query
        ...
        if response_code ==200:
            ...
            if response_body_dict['total']==0:
                ...
                movie_total=math.ceil(response_body_dict['total'] / int(display))
                if movie_total >=1000:
                    movie_count=1000
                else:
                    movie_count=movie_total
                print("영화 전체수:" +str(movie_total))
                print("영화 갯수:" +str(movie_count))
        return movie_count
    def get_movie_post(query,display, start_index,sort):
        global no, fs
        encode_query=urllib.parse.quote(query)
        ...
        if response_code ==200:
            ...
            for item_index in range(0, len(response_body_dict['items'])):
                try:
                    ...
                    moving_name=response_body_dict['items'][item_index]['movingname']
    ...
    if __name__ =='__main__':
        ...
        movie_count=get_movie_count(query, display)
        for start_index in range(start, movie_count +1,display):
            get_movie_post(query,display,start_index,sort)
        ...
        if response_code ==200:
            ...
            if response_body_dict['total']==0:
                ...
                if kin_total >=1000:
                    ...
                    kin_count=kin_total
                print("영화 전체수:" +str(kin_total))
                print("영화 갯수:" +str(kin_count))
        return kin_count
    ...
    if __name__ =='__main__':
        ...
        fs=open(query+".txt",'a',encoding='utf-8')
        kin_count=get_kin_count(query, display)
        for start_index in range(start, kin_count +1,display):
